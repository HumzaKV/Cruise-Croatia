...
// $(document).on('keyup', '#input_20_21', function(){
    
//                 var number = $(this).val();
//             if (number.length <= 13) {
//                 if (number.length == 3 || number.length == 7 || number.length == 11 || number.length == 14) {
//                     $(this).val($(this).val() + '-');
//                 }
//             }
//     })
...
<div id="contact_quote" class="box_1 style_box inquire_sec">
	<ul>
		...
		<li><label>Cabin:</label><span><span>Quantity <?= $cabins; ?> x Lower Deck Cabins </span></li>
		<input type="hidden" class="paas" value="<?php echo $_POST['passenger'] ?>">
		<li><label>Passengers:</label><span><span>Quantity <?= $_POST['passenger']; ?> x  Passengers at </span><strong><?= $passengerCost; ?></strong></li>
		<?php echo $ext_data; ?>
	</ul>
