...
if( $ext ):
	...
	echo '<div class="ex_style">';
		...
		if(!empty($price['sale_price'])){
			printf('<span class="sale-price">Full %spp</span>',_currency_format($price['regular_price'], true));
			printf('<span class="reg-price">From %s</span>',_currency_format($price['sale_price'], true));
		}
		else
